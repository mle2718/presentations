...
- Tune (select *mtry*)  train a Random Forest on 90% of our data  (2015-2024)
  ...
  - Very loosely, $Brier=\frac{1}{2n} \sum \sum (y- \hat{P}(Class))^2$
...
true<-nocluster_testing_predictions %>%
  mutate(market_desc=as.character(market_desc))%>%
    group_by(stockarea,market_desc) %>%
    summarise(across(c(true_pounds), sum)) 
...
knitr::kable(mkt_preds, caption='Predictions on the holdout sample',format.args = list(big.mark = ","), digits=0)  
```
...
knitr::kable(mkt_preds, caption='Predictions on the full sample',format.args = list(big.mark = ","), digits=0)  
``` 
---



#  Out of SamplePredictions
...
knitr::kable(mkt_preds, caption='Out of Sample Predictions',format.args = list(big.mark = ","), digits=0)  
``` 
