...
- 7-8% of total black sea bass landings during this time
...
- Train a random forest classification model using transaction-level data  `r Cite(myBib, "Breiman2001")`.  Predict whether an observation is Jumbo, Large, Medium, or Small.
...
# Validation
.pull-left[
Receiver Operating Characteristic Curve for our 10% validation hold out sample.
...
* True positive rate (vertical) vs false positive rate (horizontal) for every possible probability cutoff.
...
ggplot(test, aes(x=year, y=predicted, group=market_desc))+
    geom_line() +
...
# knitr::kable(mkt_preds, caption='Predicted Class Membership of the Unclassified Market category, by year and stock area',format.args = list(big.mark = ","), digits=0)  
``` 
...
<img src="./20250612_EconomicInformedStockAssessments/images/length_frequencies_by_market_category_2018_2022.png" alt="Length Frequencies by Market Category, 2018-2022" height="400" width="400"  />
]
...
econ_informed<-mkt_preds %>%
  ...
  select(-predicted,-total) 
  

econ_informed$percentage=paste0(econ_informed$percentage,"%")


knitr::kable(econ_informed, caption='Predictions Market Categories of the Unclassified Fish, 2018-2022 (mt)',format.args = list(big.mark = ","), digits=0, align=c("l",rep('r',times=4)))  
